...
public interface DbCategoryRepository extends JpaRepository<DbCategory, Integer> {
    ...
    @Query("""
            SELECT ct FROM DbCategory ct
            INNER JOIN DbUserCategory uc ON uc.dbUserCategoryId.dbCategory = ct
            INNER JOIN DbUser us ON uc.dbUserCategoryId.dbUser = us
            WHERE us.documentNumber = :document AND ct.categoryName = :categoryName""")
    DbCategory getCategoryByIdAndName(@Param(value = "document") String userDocument, @Param(value = "categoryName") String categoryName);
    ...
    @Query("""
            ...
            """)
    List<DbCategory> getAllCategoryByDocument(@Param(value = "document") String userDocument);
    ...
    DbCategory getAllCategoryById(@Param(value = "document") String userDocument, @Param(value = "idCategory") Integer idCategory);

    @Transactional
    @Modifying
    @Query("""
            UPDATE DbCategory ct SET
